...
export const DefaultGridPOSLayout: React.FC<DefaultGridPOSLayoutProps> = ({
  ...
  const handleCartUpdateQty = (productId: string, qty: number) => {
    ...
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative" dir="rtl">
      ...
      >
          ...
          {paginatedProducts.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 max-w-sm mx-auto">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-primary text-on-primary text-xs font-bold hover:bg-primary/90 transition-all flex items-center gap-2 shadow-sm cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-3.5">
              {paginatedProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    {showProductImages && (
                      <div className="h-[60%] w-full relative overflow-hidden bg-surface-container/60 shrink-0">
                        ...
                        {product.image ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-surface-container to-surface-container-high text-on-surface-variant/40 group-hover:text-primary transition-colors">
                    ...
                    <div className="p-3 sm:p-3.5 flex flex-col justify-between flex-1 bg-surface-container-low/95 border-t border-outline-variant/15">
                      <div>
                        <h4 className="text-xs sm:text-sm font-black text-on-surface line-clamp-1 group-hover:text-primary transition-colors leading-snug">
                          {product.name}
                        </h4>
                      ...
                      <div className="flex items-end justify-between pt-1 border-t border-outline-variant/10 mt-1">
                        <div>
                          <span className="text-[10px] text-on-surface-variant block font-tajawal">السعر</span>
                          <span className="text-sm sm:text-base font-black text-primary font-mono">
                            {formatNumber(product.retailPrice)} دج
                          </span>
                        </div>
                        <div className="w-8 h-8 rounded-xl bg-primary/10 text-primary flex items-center justify-center shadow-2xs group-hover:bg-primary group-hover:text-on-primary transition-all">
                          <Plus className="w-4 h-4" />
                        </div>
                      </div>
                    </div>
            ...
            <div className="divide-y divide-outline-variant/10 bg-surface-container-low rounded-2xl border border-outline-variant/15 overflow-hidden">
              {paginatedProducts.map((product) => {
                const isOutOfStock = (product.quantity ?? 0) <= 0;
                return (
                  <div
                    key={product.id}
                    onClick={() => !isOutOfStock && onAddProduct(product)}
                    className="p-3 hover:bg-surface-container/60 transition-colors flex items-center justify-between gap-3 cursor-pointer select-none"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      {showProductImages && (
                        <div className="w-10 h-10 rounded-xl bg-surface-container flex items-center justify-center shrink-0 overflow-hidden border border-outline-variant/20">
                          {product.image ? (
        ...
        <div className="p-3 bg-surface-container-low/90 backdrop-blur-xs border-t border-outline-variant/15 flex items-center justify-between text-xs text-on-surface-variant shrink-0 shadow-2xs">
          <button
            ...
            className="px-4 py-1.5 rounded-xl bg-surface-container hover:bg-surface-container-high border border-outline-variant/20 text-xs font-bold text-on-surface disabled:opacity-40 transition-all shadow-2xs cursor-pointer"
          >
            السابق
          </button>
      ...
      >
        ...
        <div className="p-3 bg-surface-container-low border-b border-outline-variant/20 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 transition-colors cursor-pointer"
        ...
        <POSCartContainer
          ...
          onEditPrice={onUpdatePrice}
          formatMoney={formatMoney}
          className="flex-1 min-h-0 border-none shadow-none rounded-none"
        />

        {/* Sidebar Action Bar for standard sidebar layout */}
        {posLayout !== 'bottom' && (
          <POSActionBar
...
export default DefaultGridPOSLayout;

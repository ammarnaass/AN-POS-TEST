...
interface UsePOSCartActionsParams {
  products: Product[];
  packs: any[];
...
export function usePOSCartActions({
  ...
  const handleAddProduct = useCallback(
    (product: any, customPrice?: number, explicitQty: number = 1) => {
      ...
      if (isPack) {
        ...
        const isTerminal = posLayout === 'terminal' || posLayout === 'advanced' || posLayout === 'design7';
        const isWholesaleTier = priceTier === '3' || (!['1', '2', '4'].includes(priceTier) && isWholesaleActive);

        // فحص وجود الباقة مسبقاً في السلة
        const existing = currentCart.find(
          (item) => String(item.productId) === `pack-${packId}` || (item.isPack && String(item.packId) === String(packId))
        );
        ...
        if (existing) {
          ...
          const price =
            customPrice !== undefined
              ? customPrice
              : product.unitPrice ?? resolveUnitPrice(product, targetQtyToAdd, promotions, isWholesaleActive, priceTier);

          const pkgSize = product.packageSize ? parseInt(String(product.packageSize), 10) : 0;
          const isPackProd = pkgSize > 1;
          const isWholesaleTier = priceTier === '3' || (!['1', '2', '4'].includes(priceTier) && isWholesaleActive);

          // إدخال المنتج دائماً بالكمية المطلوبة صراحة (1 افتراضياً) لتفادي فرض العبوة قسراً
          addItem({
            productId: prodId,
            name: product.name,
            qty: targetQtyToAdd,
  ...
  const handleExternalScan = useCallback(
    async (code: string, scanQty: number = 1, extraData?: { fromMobile?: boolean; product?: any }) => {
      ...
      const result = await parseAndAddScannedCode(trimmedCode, {
        ...
        addItem: (item) => {
          // توجيه الإضافة عبر handleAddProduct لضمان توحيد منطق السلة وفحص الأرصدة
  ...
  const handleApplyWeight = useCallback(
    (weightInKg: number, targetProduct?: Product) => {
      ...
      // إذا لم يُمرر المنتج، نأخذ آخر صنف نشط في السلة
      ...
      const roundedWeight = Math.round(weightInKg * 1000) / 1000; // تقريب بالجرام

      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {activeTab === 'general' && (
          <div className="space-y-6">
            ...
            <div className="bg-surface-container-low rounded-3xl border border-outline-variant/20 p-6 shadow-sm space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-outline-variant/15">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold font-cairo text-on-surface">معلومات وهُوِيّة المؤسسة</h2>
                    <p className="text-xs text-on-surface-variant">البيانات التعريفية للمتجر التي تظهر في الفواتير والتقارير</p>
                  </div>
                </div>
              </div>

              {/* رفع الشعار واسم المتجر */}
              <div className="flex flex-col sm:flex-row items-center gap-6 p-5 bg-surface-container rounded-2xl border border-outline-variant/15">
                <div className="relative w-24 h-24 rounded-2xl border-2 border-dashed border-primary/30 flex items-center justify-center overflow-hidden bg-surface-container-low shrink-0 shadow-inner group">
                  {settings.shopLogo ? (
                    <>
                      <img src={settings.shopLogo} alt="شعار المؤسسة" className="w-full h-full object-contain" />
                      <button
                        type="button"
                    ...
                    <Store className="w-10 h-10 text-on-surface-variant/40" />
                  )}
                </div>
                <div className="flex-1 space-y-2 text-center sm:text-right">
                  <h3 className="text-sm font-bold text-on-surface">شعار المتجر الرسمي</h3>
                  <p className="text-xs text-on-surface-variant leading-relaxed">
                    يتم تضمين الشعار تلقائياً في رأس فواتير الكاشير والفواتير الرسمية A4 و A5 (PNG أو JPG حتى 2MB)
                  </p>
                  <label className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-on-primary rounded-xl text-xs font-bold hover:bg-primary/90 transition-all cursor-pointer shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { label: 'اسم المؤسسة / المتجر', key: 'shopName', value: settings.shopName, placeholder: 'مثال: سوبرماركت البركة', type: 'text' },
                  { label: 'النشاط التجاري / الوصف', key: 'shopDescription', value: settings.shopDescription, placeholder: 'مثال: تجارة المواد الغذائية العامة بالتجزئة والجملة', type: 'text' },
                  { label: 'المدينة / الولاية', key: 'city', value: settings.city, placeholder: 'مثال: الجزائر العاصمة', type: 'text' },
                  { label: 'العنوان الكامل', key: 'shopAddress', value: settings.shopAddress, placeholder: 'مثال: شارع الاستقلال، عمارة 12', type: 'text' },
                  { label: 'رقم الهاتف الرئيسي', key: 'phone', value: settings.phone, placeholder: '05XX XX XX XX', type: 'tel' },
                  { label: 'الهاتف الثابت / الثانوي', key: 'shopPhone2', value: settings.shopPhone2, placeholder: '023 XX XX XX', type: 'tel' },
                  { label: 'البريد الإلكتروني', key: 'shopEmail', value: settings.shopEmail, placeholder: 'contact@shop.dz', type: 'email' },
                ].map((f) => (
                  <div key={f.key}>
                    <label className="block text-xs font-bold text-on-surface mb-1.5">{f.label}</label>
                    <input
                      type={f.type}
            ...
            {/* === البيانات القانونية والضريبية الجزائرية === */}
            <div className="bg-surface-container-low rounded-3xl border border-outline-variant/20 p-6 shadow-sm space-y-5">
              <div className="flex items-center gap-3 pb-3 border-b border-outline-variant/15">
                <div className="w-9 h-9 rounded-xl bg-blue-500/10 text-blue-600 flex items-center justify-center">
                  <ShieldCheck className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-bold font-cairo text-on-surface">البيانات القانونية والجبائية (Algerian Fiscal IDs)</h3>
                  <p className="text-xs text-on-surface-variant">المعلومات الإلزامية التي تظهر في فواتير A4 / A5 والوصولات التجارية الرسمية</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { label: 'السجل التجاري (RC)', key: 'commercialRegister', value: settings.commercialRegister, placeholder: '16/00-1234567B' },
                  { label: 'الرقم الجبائي (NIF)', key: 'taxNumber', value: settings.taxNumber, placeholder: '001616012345678' },
                  { label: 'رقم المادة الجبائية (AI)', key: 'taxArticle', value: settings.taxArticle, placeholder: '16012345678' },
                  { label: 'رقم التعريف الإحصائي (NIS / ART)', key: 'taxId', value: settings.taxId, placeholder: '123456789' },
                ].map((f) => (
                  <div key={f.key}>
                    <label className="block text-xs font-bold text-on-surface mb-1.5">{f.label}</label>
                    <input
            ...
            {/* === العملة، الضرائب، والتنسيقات الإقليمية === */}
            <div className="bg-surface-container-low rounded-3xl border border-outline-variant/20 p-6 shadow-sm space-y-5">
              <div className="flex items-center gap-3 pb-3 border-b border-outline-variant/15">
                ...
                <div>
                  <h3 className="text-base font-bold font-cairo text-on-surface">العملة والخيارات الإقليمية</h3>
                  <p className="text-xs text-on-surface-variant">ضبط العملة الرئيسية، نسبة TVA، وتنسيقات التاريخ والأرقام</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-on-surface mb-1.5">نسبة الضريبة الافتراضية TVA (%)</label>
                  <input
                    type="number"
                    min="0"
                    max="100"
                ...
                <div>
                  <label className="block text-xs font-bold text-on-surface mb-1.5">لغة واجهة النظام</label>
                  <select
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="YYYY-MM-DD">YYYY-MM-DD (سنة-شهر-يوم)</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-on-surface mb-1.5">فاصل الآلاف</label>
                  <select
                    value={settings.thousandsSeparator || ','}
                    onChange={(e) => handleSaveSettings({ thousandsSeparator: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl bg-surface-container border border-outline-variant/20 text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                  >
                    <option value=",">فاصلة (1,000.00)</option>
                    <option value=" ">مسافة (1 000.00)</option>
                    <option value=".">نقطة (1.000,00)</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-on-surface mb-1.5">فاصل الكسور العشرية</label>
                  <select
                    value={settings.decimalSeparator || '.'}
                    onChange={(e) => handleSaveSettings({ decimalSeparator: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl bg-surface-container border border-outline-variant/20 text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                  >
                    <option value=".">نقطة (0.00)</option>
                    <option value=",">فاصلة (0,00)</option>
                  </select>
                </div>
              </div>
            </div>

            {/* === إدارة العملات الإضافية === */}
            <div className="bg-surface-container-low rounded-3xl border border-outline-variant/20 p-6 shadow-sm space-y-4">
              <div className="flex items-center gap-3 pb-3 border-b border-outline-variant/15">
                <div className="w-9 h-9 rounded-xl bg-purple-500/10 text-purple-600 flex items-center justify-center">
                  <CircleDollarSign className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-bold font-cairo text-on-surface">إدارة العملات وأسعار الصرف</h3>
                  <p className="text-xs text-on-surface-variant">التعامل بعملات متعددة في المبيعات والمشتريات</p>
                </div>
              </div>

              <div className="space-y-2">
                {settings.currencies.map((currency) => (
                  <div key={currency.code} className="flex items-center justify-between p-3.5 bg-surface-container rounded-2xl border border-outline-variant/15">
                    <div className="flex items-center gap-3">
                      <span className="w-10 h-10 rounded-xl bg-primary/10 text-primary font-bold text-base flex items-center justify-center">
                        {currency.symbol}
                      </span>
                      <div>
                        <p className="text-sm font-bold text-on-surface">{currency.code}</p>
                        <p className="text-xs text-on-surface-variant">سعر الصرف مقابل العملة الأساسية: {currency.rateToBase}</p>
                      </div>
                      {currency.code === settings.baseCurrency.replace(/[^A-Z]/g, '') && (
                        <span className="bg-primary/10 text-primary px-2.5 py-0.5 rounded-full text-xs font-bold">الأساسية</span>
                      )}
                    </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <input
                  ...
                  className="px-4 py-2.5 rounded-xl bg-surface-container border border-outline-variant/20 text-sm text-on-surface flex-1 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary uppercase"
                  maxLength={5}
                />
                <input
                ...
                <input
                  ...
                  className="px-4 py-2.5 rounded-xl bg-surface-container border border-outline-variant/20 text-sm text-on-surface flex-1 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-bold"
                />
                <button
                  type="button"
                  onClick={handleAddCurrency}
                  className="px-5 py-2.5 bg-primary text-on-primary rounded-xl text-xs font-bold hover:bg-primary/90 transition-all flex items-center justify-center gap-1.5 shadow-sm"
                >
            ...
            <div className="bg-surface-container-low rounded-3xl border border-outline-variant/20 p-6 shadow-sm space-y-4">
              <div className="flex items-center gap-3 pb-3 border-b border-outline-variant/15">
                ...
                <div>
                  <h3 className="text-base font-bold font-cairo text-on-surface">فئات وتصنيفات المصاريف</h3>
                  <p className="text-xs text-on-surface-variant">تبويب المصاريف التشغيلية للمتجر (إيجار، رواتب، كهرباء، صيانة...)</p>
                </div>
              </div>

              <div className="flex flex-wrap gap-2">
                {settings.expenseCategories.map((category: string) => (
                  <span key={category} className="flex items-center gap-2 px-3.5 py-1.5 bg-surface-container rounded-xl text-xs font-bold text-on-surface border border-outline-variant/15 shadow-xs">
                    <span>{category}</span>
                    <button
                      type="button"
              ...
              <div className="flex gap-2 pt-2">
                <input
                  ...
                  placeholder="أدخل اسم فئة مصاريف جديدة..."

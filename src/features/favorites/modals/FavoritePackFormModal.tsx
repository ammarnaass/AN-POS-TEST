...
import {
  QUICK_PIECES_OPTIONS,
  QUICK_UNIT_OPTIONS,
  PACK_TYPE_OPTIONS,
} from '../constants/favoritePackOptions';

export interface FavoritePackFormModalProps {
  isOpen: boolean;
  mode: 'create' | 'edit';
  ...
  setPackType: (type: FavoritePackType) => void;
  minWholesaleQty: number;
  setMinWholesaleQty: (qty: number) => void;
  searchQuery: string;
...
export const FavoritePackFormModal: React.FC<FavoritePackFormModalProps> = ({
  ...
  const standardPackPrice = baseUnitPrice * Math.max(1, piecesCount);
  const currentPackPrice = Number(packPrice) || 0;
  const hasDiscount = selectedProduct && currentPackPrice > 0 && currentPackPrice < standardPackPrice;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-surface-container-low dark:bg-slate-900 border border-outline-variant/20 dark:border-slate-800 rounded-3xl w-full max-w-2xl p-5 sm:p-6 shadow-2xl space-y-4 max-h-[92vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold font-cairo text-on-surface dark:text-white flex items-center gap-2">
                <span>{isEdit ? 'تعديل بيانات وسعر العبوة' : 'إنشاء عبوة سريعة من منتج تجزئة'}</span>
                <span
                ...
                >
                  {isEdit ? 'تعديل شامل' : 'خاص بتصميم 5'}
                </span>
              </h3>
              <p className="text-[11px] text-on-surface-variant dark:text-slate-400 mt-0.5">
                {isEdit
                  ? 'تعديل المنتج الأصلي، مواصفات التعبئة، الاسم، سعر البيع، أو تصنيف المفضلة'
                  : 'تحويل سلع التجزئة إلى كراتين وباقات تُباع باللمس بدون باركود مع خصم المخزون التلقائي'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={onSubmit} className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-4">
          {/* 1. اختيار وتغيير منتج التجزئة الأساسي */}
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300">
                1. منتج التجزئة الأساسي المرتبط بالعبوة: <span className="text-rose-500">*</span>
              </label>
              {selectedProduct && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span>تغيير المنتج الأساسي</span>
                </button>
              )}
            </div>
              ...
              <div className="space-y-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="ابحث باسم المنتج أو الباركود لربط العبوة به..."
          ...
          {/* 2. عدد القطع واسم وحدة التعبئة */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {/* عدد القطع في العبوة */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300">
                  2. عدد القطع في العبوة: <span className="text-rose-500">*</span>
                </label>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => onChangePiecesCount(Math.max(1, piecesCount - 1))}
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-surface-container dark:bg-slate-800 border border-outline-variant/20 dark:border-slate-700 rounded-xl text-xs font-mono font-bold text-center text-on-surface dark:text-white focus:outline-none focus:border-primary"
                  required
                />
                <button
                  type="button"
                  onClick={() => onChangePiecesCount(piecesCount + 1)}
                  className="w-8 h-8 rounded-lg bg-surface-container dark:bg-slate-800 border border-outline-variant/20 dark:border-slate-700 font-bold text-sm flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer"
                >
                  +
                </button>
              ...
              {/* أزرار سريعة موحدة للقطع */}
              <div className="flex items-center gap-1 flex-wrap pt-0.5">
                {QUICK_PIECES_OPTIONS.map((count) => (
                  <button
                    key={count}
                    type="button"
                    onClick={() => onChangePiecesCount(count)}
                    className={`px-2 py-0.5 text-[10px] font-mono font-bold rounded-lg border transition cursor-pointer ${
                      piecesCount === count
            ...
            <div className="space-y-1.5">
              ...
              {/* أزرار سريعة موحدة للوحدات */}
              <div className="flex items-center gap-1 flex-wrap pt-0.5">
                {QUICK_UNIT_OPTIONS.map((unit) => (
                  <button
                    key={unit}
                    type="button"
          ...
          {/* 3. اسم العبوة على الكاشير مع زر التوليد الآلي */}
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300">
              ...
              {selectedProduct && (
                <button
                  ...
                  className="text-[11px] font-bold text-primary hover:underline flex items-center gap-1 cursor-pointer"
                  title="توليد الاسم تلقائياً من اسم المنتج ووحدة التعبئة وعدد القطع"
                >
                  <RefreshCw className="w-3 h-3" />
                  <span>توليد الاسم تلقائياً</span>
                </button>
              )}
            </div>
          ...
          {/* 4. سعر بيع العبوة مع الحساب التلقائي وشارة التخفيض */}
          <div className="p-3.5 bg-surface-container dark:bg-slate-800/80 border border-outline-variant/20 dark:border-slate-700/60 rounded-2xl space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300">
              ...
              {selectedProduct && (
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono text-slate-400">
                    الحساب القياسي: {formatMoney(standardPackPrice)} د.ج
                  </span>
                  <button
                    type="button"
                    onClick={onAutoCalculatePrice}
                    className="text-[11px] font-bold text-emerald-700 dark:text-emerald-300 hover:underline px-2 py-0.5 bg-emerald-100 dark:bg-emerald-950/60 rounded border border-emerald-200 dark:border-emerald-800 flex items-center gap-1 cursor-pointer"
                  >
            ...
            {/* تفاصيل السعر والشارة التشجيعية */}
            <div className="flex items-center justify-between flex-wrap gap-2 pt-1">
              {currentPackPrice > 0 && piecesCount > 0 && (
                <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-md">
                  سعر الحبة داخل العبوة: {formatMoney(currentPackPrice / piecesCount)} د.ج
                </span>
              )}

              {hasDiscount && (
                <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-100/70 dark:bg-emerald-950/70 px-2 py-0.5 rounded-md">
                  ★ سعر تشجيعي بتخفيض {formatMoney(standardPackPrice - currentPackPrice)} د.ج للكرتونة
                </span>
              )}
            </div>
          </div>

          {/* 5. تصنيف نوع الباقة والحد الأدنى للجملة */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300 block">
                6. نوع الباقة / التسعير:
              </label>
              <select
                value={packType}
                onChange={(e) => setPackType(e.target.value as FavoritePackType)}
                className="w-full px-3 py-2 bg-surface-container dark:bg-slate-800 border border-outline-variant/20 dark:border-slate-700 rounded-xl text-xs font-bold text-on-surface dark:text-white focus:outline-none focus:border-primary"
              >
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300 block">
                7. الحد الأدنى لتطبيق سعر الجملة:
              </label>
              <input
                type="number"
                min="1"
                value={minWholesaleQty}
                onChange={(e) => setMinWholesaleQty(Math.max(1, parseInt(e.target.value) || 1))}
          ...
          {/* 6. الباركود وتصنيف المفضلة */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {/* باركود العبوة */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300">
                  8. باركود العبوة (اختياري):
                </label>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-on-surface-variant dark:text-slate-300 block">
                9. تصنيف المفضلة التابع له: <span className="text-rose-500">*</span>
              </label>
              <select
                value={targetCatId}
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-outline-variant/15 dark:border-slate-800 shrink-0">
            ...
            <button
              ...
              className={`px-5 py-2 text-xs font-bold text-white rounded-xl shadow-xs transition cursor-pointer flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed ${
                isEdit
